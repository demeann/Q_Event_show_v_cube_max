...
def _round_windows(game_start_msk: date) -> dict[RoundCode, tuple[str, date, date]]:
    ...
    Тур 1: дни +0…+3. Тур 2: +4…+6. Тур 3: **+6…+8** (старт в 10:00 МСК = 07:00 UTC).
    Последний день R2 и первый день R3 совпадают по календарю; с 10:00 МСК активен R3
    (см. ``get_playable_round_now`` — выбирается тур с более поздним ``starts_at``).
    """
    return {
        RoundCode.R1: (
